...
export default function createApiRouter(options: ApiRouterOptions) {
    ...
    router.get("/views/:datasetId", function(req, res) {
        const datasetId = req.params.datasetId;
        const userId = getUserId(req, options.jwtSecret).toString()
        let view = {"datasetId": datasetId, "user": userId, eventType: 1 }
        // Add view to db
        database.createView(view);
        // Return current views for given dataset
        handlePromise(res, database.getViews(datasetId));
    });

    router.post("/views", async function(req, res) {
        try {
            const view = await database.createView(req.body);
            res.json(view);
